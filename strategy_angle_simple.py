...
def strategy_angle_simple(state):
	...
	x_translate = state["bot_pos"].x + (state["bot_pos"].x - state["tag_loc"].x) * 2
	
	angle = state["tag_loc"].asPolar().addVector(CartVertex2D(-x_translate, -state["bot_pos"].y)).asPolar().a
...
ani = FuncAnimation(fig, partial(animate, state=curr_state, strategy=strategy_angle_simple), interval=300)
plt.show()
